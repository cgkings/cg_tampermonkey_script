...
(function () {
    ...
    class EmbyAPI {
        constructor() {
            ...
            // 加载localStorage缓存
            try {
                const cachedData = localStorage.getItem('embyJumpCache');
                if (cachedData) {
                    const parsed = JSON.parse(cachedData);
                    if (parsed && typeof parsed === 'object') {
                        Object.entries(parsed).forEach(([key, value]) => {
                            if (value && value.timestamp && (Date.now() - value.timestamp < 86400000)) {
                                this.cache.set(key, value.data);
                            }
                        });
                    }
                }
            } catch (e) { console.error('缓存读取错误:', e); }
        }
        ...
        saveCache() {
            try {
                ...
                this.cache.forEach((value, key) => {
                    cacheObj[key] = { data: value, timestamp: Date.now() };
                });
                localStorage.setItem('embyJumpCache', JSON.stringify(cacheObj));
            } catch (e) { console.error('缓存保存错误:', e); }
        }
